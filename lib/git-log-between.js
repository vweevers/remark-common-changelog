...
async function getCommits (opts) {
  ...
  const resolveTags = (tags) => Promise.all(tags.map(t => resolveCommitish(dir, gitdir, t)))
  const tags = new Set(opts.ignoreTags ? [] : await git.listTags({ fs, dir, gitdir }).then(resolveTags))
