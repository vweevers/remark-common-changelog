...
import versionCommitRe from './version-commit-re.js'
...
async function getCommits (opts) {
  ...
  const ignoreTags = opts.ignoreTags === true
  ...
  let lastCommitOid = lowerCommitOid
  ...
  while (!complete && commits.length < limit) {
    ...
    for (const { oid, commit } of chunk) {
      // Workaround: stop if a tag is encountered, in cases where `gt` is a tag
      // on a commit on another branch, which we therefore would never see here.
      // Resolving all tags to an oid can be slow, so instead match on message.
      if (versionCommitRe.test(commit.message) && !ignoreTags) {
        lastCommitOid = oid
        complete = true
        break
      }
  ...
  if (opts.submodules && fs.existsSync(path.join(dir, '.gitmodules'))) {
    ...
    // TODO: we don't always have a lowerCommitOid/lastCommitOid
    for (const { commit, submodule } of modules(dir, lastCommitOid)) {
    ...
    for (const [submodule, { gt, lte }] of submodules) {
      ...
      const subcommits = await getCommits({
        cwd,
        gitdir,
        gt,
        lte,
...
function modules (dir, ref) {
  if (!ref) {
    throw new Error('Cannot list submodules without ref')
  }
...
async function tryGetRepo (dir, gitdir) {
  ...
  try {
    let host = Githost.fromPkg(dir, { optional: true })
    if (host) return host.slug({ commitish: false })

    const remotes = await git.listRemotes({ fs, dir, gitdir })
    const origin = remotes.filter(r => r.remote === 'origin')[0]

    if (origin && origin.url) {
      host = Githost.fromUrl(origin.url, { optional: true })
      if (host) return host.slug({ commitish: false })
    }
  } catch (err) {
    console.error(err)
